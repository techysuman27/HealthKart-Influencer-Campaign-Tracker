...
if not insights_df.empty:
    ...
    with col2:
        ...
        if low_roi > 0:
            st.warning(f"📉 **{low_roi} influencers** with negative ROI")
        
        # Low engagement influencers
        low_engagement = len(insights_df[insights_df['avg_engagement'] < 1.0])
        if low_engagement > 0:
            st.warning(f"😴 **{low_engagement} influencers** with engagement < 1%")
        
        # Underutilized influencers
        underutilized = len(insights_df[insights_df['total_posts'] < 3])
        if underutilized > 0:
            st.warning(f"💤 **{underutilized} influencers** with < 3 posts")

else:
    st.info("No influencer data available for the selected filters.")